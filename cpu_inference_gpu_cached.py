#!/usr/bin/env python3
"""
DeepSeek-Math-V2 inference with GPU compute AND expert caching.
- Weights loaded from disk, cached in RAM with LRU eviction
- Matrix multiplications on GPU
- Expert LRU cache to avoid repeated disk loads
"""
...
# Expert cache settings - use spare RAM
EXPERT_CACHE_SIZE_GB = 80  # Max GB for expert cache
...
class ExpertLRUCache:
    """LRU cache for expert weights to minimize disk I/O."""
    ...
    def get(self, layer: int, expert_id: int) -> Optional[Dict[str, torch.Tensor]]:
        """Get expert from cache, returns None if not cached."""
        key = (layer, expert_id)
    ...
    def put(self, layer: int, expert_id: int, tensors: Dict[str, torch.Tensor]):
        """Add expert to cache, evicting old entries if needed."""
        key = (layer, expert_id)

        # Calculate size of this expert
        expert_size = sum(t.numel() * t.element_size() for t in tensors.values())

        # Evict until we have space
        while self.current_size + expert_size > self.max_size and self.cache:
            oldest_key, oldest_tensors = self.cache.popitem(last=False)
            old_size = sum(t.numel() * t.element_size() for t in oldest_tensors.values())
    ...
    def stats(self) -> str:
        ...
        return f"Cache: {len(self.cache)} experts, {self.current_size/1e9:.1f}GB, hit rate: {hit_rate:.1f}%"


class TensorLoader:
    """Memory-mapped tensor loading with expert caching."""

    def __init__(self, model_path: str, expert_cache_gb: float = EXPERT_CACHE_SIZE_GB):
        self.model_path = model_path
        self.index = self._load_index()
        self.file_handles: Dict[str, safe_open] = {}
        self.cache: Dict[str, torch.Tensor] = {}
        self.cache_size_limit = 100 * 1024 * 1024 * 1024  # 100GB for base weights
        self.current_cache_size = 0

        # Expert LRU cache
        self.expert_cache = ExpertLRUCache(int(expert_cache_gb * 1024 * 1024 * 1024))
    ...
    def load_expert(self, layer: int, expert_id: int) -> Dict[str, torch.Tensor]:
        """Load expert with LRU caching."""
        # Check cache first
        cached = self.expert_cache.get(layer, expert_id)
        ...
        # Add to cache
        if tensors:
            self.expert_cache.put(layer, expert_id, tensors)
...
class DeepSeekV32CachedGPU:
    """Model with GPU compute and expert caching."""
    ...
    def mlp_moe_gpu_cached(self, x: torch.Tensor, layer: int) -> torch.Tensor:
        """MoE MLP with GPU compute and expert caching."""
        ...
        for expert_id in unique_experts:
            ...
            self.expert_load_time += time.time() - t0
    ...
    def transformer_block(self, x: torch.Tensor, layer: int, start_pos: int) -> torch.Tensor:
        ...
        if layer < self.args.n_dense_layers:
            ...
            mlp_out = self.mlp_moe_gpu_cached(h, layer)
    ...
    def generate(self, prompt: str, max_new_tokens: int = 50, temperature: float = 0.7) -> str:
        ...
        print(f"Expert cache: {self.loader.expert_cache.stats()}")
        ...
        if token_times:
            ...
            print(f"\nGeneration stats:")
            print(f"  Avg time per token: {avg_time:.2f}s")
            print(f"  Expert load time: {self.expert_load_time:.1f}s")
            print(f"  Compute time: {self.compute_time:.1f}s")
            print(f"  {self.loader.expert_cache.stats()}")
